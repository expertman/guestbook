...
pipeline {
    ...
    stages {
        ...
        stage ('JMeter LoadTest') {
            agent { label 'agent1' }
            steps { 
                sh '~/jmeter/bin/jmeter.sh -j jmeter.save.saveservice.output_format=xml -n -t ~/src/main/jmx/guestbook_loadtest.jmx -l loadtest_result.jtl' 
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'devtrainer.naver.com@gmail.com')
